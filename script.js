...
saveButton.addEventListener('click', () => {
    ...
    link.download = 'pixel-art.png'; // Задаем имя файла
    link.href = canvas.toDataURL('image/png');

    document.body.appendChild(link); // Добавляем ссылку в DOM

    // Обработчик события для ПК
    link.click();

    // Обработчик события для мобильных устройств
    link.addEventListener('touchend', (e) => {
        e.preventDefault(); // Предотвращаем дополнительное событие
        link.click();
        document.body.removeChild(link); // Удаляем ссылку
    }, { once: true }); // Удаляем обработчик после первого вызова
